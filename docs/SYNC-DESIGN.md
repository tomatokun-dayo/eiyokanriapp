...
### Phase 2 の実装メモ（2026-07-30 完了）

同期の起動条件は次の4系統。どれか1つが死んでも他が埋める形にしてある。

| きっかけ | 動作 | 目的 |
|----------|------|------|
| 変更から1.5秒（デバウンス） | Realtime健在なら push のみ、切れていれば pull+push | 連続入力を1回にまとめる |
| Realtime通知の受信 | 届いた1行だけを適用して再描画 | 相手の記録が無操作で出る |
| ログイン直後 / タブ復帰 / オンライン復帰 | pull+push | 通知を取りこぼした分の追いつき |
| Realtime再接続（切断を挟んだ場合のみ） | pull+push | 切断中に起きた変更の回収 |

実装中に判明し、併せて直した土台の問題:

1. **送信中の変更が消える**: `pushQueue` が await 前に読んだメタ情報でキューを保存し直していたため、
   送信の最中に積まれた変更を巻き戻していた。送信後に読み直し、`updatedAt` が一致する行だけ削除する方式へ。
   手動同期では踏みにくいが、自動送信では入力のたびに起こりうる。
2. **Realtime の時刻形式でLWWが壊れる**: PostgREST は `2026-07-30T10:00:00+00:00` を返すが、
   Realtime(WAL) は `2026-07-30 10:00:00+00` と空白区切りで来る。文字列比較では区切り文字のコード順で
   「リモートが常に古い」と誤判定し、保留変更がある行のリモート更新を取りこぼす。`Date.parse` で比較する。
3. **無変更でも全書き戻し**: 取り込んだ行を無条件に `changedStores` へ入れていたため、
   復帰のたびに localStorage 全書き換え＋再描画が走る。キー順に依存しない比較で差分だけ適用する。

`foodPrefs` は当初の保留事項どおり同期対象のまま。運用して違和感が出たら外す。

#### 追いつき取得を差分化した理由（無料枠の転送量）

Phase 1 の pull は毎回 `select *`（全件）だった。手動同期なら1日数回なので問題にならないが、
Phase 2 はタブ復帰のたびに追いつき同期が走るため、**転送量が「蓄積行数 × 復帰回数」で伸びる**。

| | 1回の取得 | 月間（2人×1日15回復帰） | 無料枠5GBに対して |
|---|---|---|---|
| 運用1年後・全件取得 | 約3MB（1万行×300B） | 約3GB | 60%。以降も増え続ける |
| 同・差分取得 | 数KB〜 | 約0.2GB（日次の全件突合わせ込み） | 4%。行数が増えても伸びない |

そこで `updated_at > lastPullAt` の差分取得にした。ただし `updated_at` は**サーバー時刻ではなく
各端末の時計**で書かれるため、境界をそのまま使うと時計が遅れている端末の書き込みを永久に取り逃す。
対策は2段構え:

- 境界を `PULL_MARGIN_MS`（5分）手前に戻す。端末間の時計ずれの実用上の上限を見込む
- `FULL_PULL_INTERVAL_MS`（24時間）ごとに全件突き合わせへ戻す（ずれが5分を超えた場合の保険）

余裕分だけ既知の行を取り直すが、§前述の差分判定により再描画は起きない。

なお Realtime のメッセージ数は無料枠200万件/月に対し、想定利用（1日30行の変更×2端末）で
月2千件程度。同時接続数も上限200に対し2〜4。いずれも桁違いに余裕がある。
